...
def get_grants(request):
    ...
    return JsonResponse({
        'grant_types': get_grant_clr_types(clr_round, _grants) if clr_round else get_grant_types(network, _grants),
...
def get_grant_clr_types(clr_round, active_grants=None):
    ...
    for _grant_type in _grant_types:
        count = active_grants.filter(grant_type=_grant_type).count() if active_grants else 0
...
def grants_by_grant_clr(request, clr_round):
    ...
    # categories = [_category[0] for _category in basic_grant_categories(grant_type)]
    grant_types = get_grant_clr_types(clr_round)
    ...
    response = TemplateResponse(request, 'grants/clr/index.html', params)
    response['X-Frame-Options'] = 'SAMEORIGIN'
    return response
