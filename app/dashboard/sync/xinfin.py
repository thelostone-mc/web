...
def find_txn_on_xinfin_explorer(fulfillment):
    ...
    if response['message'] and response['result']:
        for txn in response['result']:
            to_address_match = txn['to'] == payeeAddress.lower() if token_name == 'XDC' else True
            if (
                txn['from'] == funderAddress.lower() and
                to_address_match and
                float(txn['value']) == float(amount) and # NOTE: This is not converted to decimals as API normalizes it
